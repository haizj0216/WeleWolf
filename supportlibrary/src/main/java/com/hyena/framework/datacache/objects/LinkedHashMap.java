...
/**
 * LinkedHashMap is an implementation of {@link Map} that guarantees iteration
 ...
 * 
 * <p>
 * Entries are kept in a doubly-linked list. The iteration order is, by default,
 * the order in which keys were inserted. Reinserting an already-present key
 * doesn't change the order. If the three argument constructor is used, and
 * {@code accessOrder} is specified as {@code true}, the iteration will be in
 * the order that entries were accessed. The access order is affected by
 * {@code put}, {@code get}, and {@code putAll} operations, but not by
 * operations on the collection views.
 * 
 * <p>
 * Note: the implementation of {@code LinkedHashMap} is not synchronized. If one
...
class LinkedHashMap<K, V> extends HashMap<K, V> {
	...
	/**
	 * A dummy entry in the circular linked list of entries in the map. The
	 * first real entry is header.nxt, and the last is header.prv. If the map is
	 * empty, header.nxt == header && header.prv == header.
	 */
	...
	/**
	 * Evicts eldest entry if instructed, creates a new entry and links it in as
	 * head of linked list. This method should call constructorNewEntry (instead
	 * of duplicating code) if the performance of your VM permits.
	 * 
	 * <p>
	 * It may seem strange that this method is tasked with adding the entry to
	 * the hash table (which is properly the province of our superclass). The
	 * alternative of passing the "next" link in to this method and returning
	 * the newly created element does not work! If we remove an (eldest) entry
	 * that happens to be the first entry in the same bucket as the newly
	 * created entry, the "next" link would become invalid, and the resulting
	 * hash table corrupt.
	 */
	@Override
	void addNewEntry(K key, V value, int hash, int index) {
		...
		LinkedEntry<K, V> newTail = new LinkedEntry<K, V>(key, value, hash,
				table[index], header, oldTail);
	...
	void addNewEntryForNullKey(V value) {
		...
		// Create new entry, link it on to list, and put it into table
		LinkedEntry<K, V> oldTail = header.prv;
		LinkedEntry<K, V> newTail = new LinkedEntry<K, V>(null, value, 0, null,
				header, oldTail);
