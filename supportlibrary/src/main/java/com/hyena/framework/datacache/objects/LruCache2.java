...
//	private static final String TAG = "LruCache2";
	...
	private int size;
	private int initSize = DEFAULT_CACHE_MAX_SIZE;
	private int maxSize = DEFAULT_CACHE_MAX_SIZE;
	private int minSize = DEFAULT_CACHE_MIN_SIZE;
	 ...
	 *            cache.
	 * @param minSize
	 *            minimum number of entries in the cache.
	 */
	public LruCache2(int maxSize, int minSize) {
		if (maxSize <= 0 || minSize >= maxSize) {
			maxSize = 0;
			minSize = 0;
		}
		this.initSize = maxSize;
...
//		LogUtil.d(TAG, "hit rate : " + getHitRate());
	...
	/*
	 * Auto trim the size of this cache to 0.8 of current size.
	 */
	public void trim() {
		this.maxSize = Math.max((int) (this.maxSize * 0.8), this.minSize);
	...
	protected void entryRemoved(boolean evicted, K key, V oldValue, V newValue) {
		synchronized (this) {
			recycleEldestEntry(oldValue);
		}
	}

	abstract public void recycleEldestEntry(V eldest);
	...
	public synchronized final String toString() {
		...
		return String.format(
				"LruCache2[maxSize=%d,hits=%d,misses=%d,hitRate=%d%%]",
				maxSize, hitCount, missCount, hitPercent);
	}

	/*
	 * Returns hit rate of this cache.
	 */
	public synchronized final int getHitRate() {
		int accesses = hitCount + missCount;
		int hitPercent = accesses != 0 ? (100 * hitCount / accesses) : 0;
		return hitPercent;
	}
	 ...
	 * 
	 * @return {@code true} if this cache has no elements, {@code false}
	 *         otherwise.
	 * @see #size()
	 */
