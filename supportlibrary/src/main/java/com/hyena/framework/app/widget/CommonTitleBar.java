...
public class CommonTitleBar extends TitleBar {
	...
	protected void initBackBtn(){
		...
		LayoutParams backBtnParams = new LayoutParams(size, size);
		...
		//textView mode
		LayoutParams backTxtParams = new LayoutParams(
				ViewGroup.LayoutParams.WRAP_CONTENT, ViewGroup.LayoutParams.WRAP_CONTENT);
		backTxtParams.addRule(CENTER_VERTICAL);
		addView(mBackTxt = getBackText(), backTxtParams);
		mBackTxt.setPadding(padding, padding, padding, padding);
	...
	protected void initTitleBar(){
		...
		//初始化主标题
		initMainTitle(titlePanel);
		//初始化副标题
		initSubTitle(titlePanel);
	}

	//init main title
	protected void initMainTitle(LinearLayout titlePanel) {
	...
	//init sub title
	protected void initSubTitle(LinearLayout titlePanel) {
	...
	protected void initTips(){
		...
		LayoutParams barTipsParams = new LayoutParams(
				size, size);
	...
	public void setMenuItems(final List<MenuItem> menuItems) {
		...
		if (menuItems != null && !menuItems.isEmpty()) {
			...
			if (menuItems.size() > 1) {
				//刷掉菜单点击事件
				mMenuImageBtn.setOnClickListener(new OnClickListener() {
					@Override
					public void onClick(View v) {
	...
	protected void showMenuList(List<MenuItem> menuList){
		...
		if (fragment != null && fragment.getRootView() != null) {
			...
			fragment.getRootView().addView(mMenuListPanel, new LayoutParams(
			...
			if (menulistPanel != null) {
				...
				mMenuListPanel.addView(maskView, new LayoutParams(
						LayoutParams.MATCH_PARENT,
						LayoutParams.MATCH_PARENT));
				...
				LayoutParams menuListPanelParams = new LayoutParams(
	...
	private void setSingleMenu(final MenuItem menuItem){
		if (menuItem.icon != 0) {
			...
			mMenuTxtBtn.setOnClickListener(new OnClickListener() {
				@Override
				public void onClick(View v) {
					if(mListener != null){
                        mListener.onMenuSelected(menuItem);
