...
public class MusicPlayer {
	...
	private boolean mResetSong = false;
	...
	public void resetSong() {
		...
		if(mSong.isOnline()) {
			...
			mBufferingThread = new BufferingThread(mSong);
			mBufferingThread.start();
		}else{
			mPlayer.setDataSource(mSong);
		}
...
//							LogUtil.v(TAG, "currentPos: " + mCurrentPos);
	...
	public long getPosition(){
		return mPlayer == null? mPlayPosition : mPlayer.getCurrentPosition();
	}
		...
		if(mPlayState == StatusCode.STATUS_BUFFING) {
			...
			if(mBufferingThread.getContentLength() == mBufferingThread.getDownloadedPos()){
				...
				if(mBufferingThread.getDownloadedPos() - playPosition >= BLOCK_SIZE){
					...
					if(DEBUG)
						LogUtil.v(TAG, "first start play, resetSong: " + mResetSong);
					if (mResetSong)
						return;
	...
	private OnPlayStateChangeListener mInnerPlayStateChangeListener = new OnPlayStateChangeListener() {
		...
		public void onPlayStateChange(int state) {
			...
			mPlayState = state;
			if (mPlayState == StatusCode.STATUS_COMPLETED
					|| mPlayState == StatusCode.STATUS_ERROR) {
				mPlayPosition = 0;
				mPlayer.reset();
			}
