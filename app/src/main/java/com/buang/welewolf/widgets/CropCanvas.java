...
public class CropCanvas extends ImageView {
	...
	private Paint mCiclePaint = null; // 画笔
	...
	public void init() {
		...
		mCiclePaint.setAntiAlias(true); // 去锯齿
		mCiclePaint.setStrokeCap(Paint.Cap.ROUND);
		mCiclePaint.setStrokeJoin(Paint.Join.ROUND);
		mCiclePaint.setColor(Color.parseColor("#4D4D4D"));
		mCiclePaint.setStyle(Paint.Style.FILL); // 将画笔的风格改为空心
		mCiclePaint.setXfermode(new PorterDuffXfermode(Mode.DARKEN)); // 因为我们先画了图所以DST_IN
	...
	private void setPressRecLoc() {
		recLT.set(ChooseArea.left - margin, ChooseArea.top - margin,
				ChooseArea.left + margin, ChooseArea.top + margin);
		recLB.set(ChooseArea.left - margin, ChooseArea.bottom - margin,
				ChooseArea.left + margin, ChooseArea.bottom + margin);
		recRT.set(ChooseArea.right - margin, ChooseArea.top - margin,
				ChooseArea.right + margin, ChooseArea.top + margin);
		recRB.set(ChooseArea.right - margin, ChooseArea.bottom - margin,
				ChooseArea.right + margin, ChooseArea.bottom + margin);
	}
	...
	private void drawCropImg(Canvas canvas) throws Exception{
		...
		canvas.drawOval(recLT, mCiclePaint);
		canvas.drawOval(recLB, mCiclePaint);
		canvas.drawOval(recRT, mCiclePaint);
		canvas.drawOval(recRB, mCiclePaint);
