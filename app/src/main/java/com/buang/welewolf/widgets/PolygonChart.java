...
//        }
    ...
    public PolygonChart(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        fillColor = array.getColor(R.styleable.PolygonChart_solid_color, DEFAULT_FILLCOLOR);
        textColor = array.getColor(R.styleable.PolygonChart_text_color, DEFAULT_TEXTCOLOR);
        dashColor = array.getColor(R.styleable.PolygonChart_dash_color, DEFAULT_BORDERCOLOR);
        init();
    }
    ...
    private void init() {
        ...
        dashPaint.setStrokeWidth(borderSize);
    ...
    private void drawPolygon(Canvas canvas) {
        ...
        canvas.drawLine(pointfList.get(pointfList.size() - 1).x, pointfList.get(pointfList.size() - 1).y, pointfList.get(0).x, pointfList.get(0).y, dashPaint);
        for (int i = 1; i < pointfList.size() - 1; i++) {
            path.lineTo(pointfList.get(i).x, pointfList.get(i).y);
            canvas.drawLine(pointfList.get(pointfList.size() - 1).x, pointfList.get(pointfList.size() - 1).y, pointfList.get(i).x, pointfList.get(i).y, dashPaint);
        }
        path.close();
