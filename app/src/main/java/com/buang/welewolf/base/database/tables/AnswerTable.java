...
//			values.put(COLUMN_ISCORRECT, answerItem.isCorrect);
	...
	public static Uri getNotifyUploadStateUri(){
		Uri uri = Uri.parse(SCHEMA + "com.knowbox.wb.provider.providers.update_" + AnswerTable.TABLE_NAME
				+ "/uploadstate");
		return uri;
	}
	
	public Uri getNotifyCorrectUri(){
		Uri uri = Uri.parse(SCHEMA + "com.knowbox.wb.provider.providers.update_" + getTableName()
				+ "/correcturi");
		return uri;
	}
