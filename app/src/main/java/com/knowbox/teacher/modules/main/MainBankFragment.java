...
import com.knowbox.teacher.base.bean.OnlineCompetitionListInfo.CompetitionItem;
...
import com.knowbox.teacher.widgets.GradientDrawableBuilder;
...
public class MainBankFragment extends BaseUIFragment<UIFragmentHelper> {
    ...
    public void onViewCreatedImpl(View view, Bundle savedInstanceState) {
        ...
        setTitleRightText();
        mListView = (LoadMoreListView) view.findViewById(R.id.competition_list);
        ...
        refreshLayout = (SwipeRefreshLayout) view.findViewById(R.id.competition_list_refresh);
        refreshLayout.setColorSchemeColors(getResources().getColor(R.color.color_text_primary));
        ...
        loadDefaultData(PAGE_FIRST);
        view.findViewById(R.id.competition_submit).setOnClickListener(new View.OnClickListener() {
            @Override
    ...
    private void setTitleRightText() {
        getUIFragmentHelper().getTitleBar().setRightMoreTxtColor(ContextCompat.getColor(getActivity(), R.color.color_text_main));
            ...
            public void onClick(View v) {
                if (null != adapter && adapter.getCount() > 0) {
                    if (currentMode == ADAPTER_MODE_NOMAL) {
                        ...
                        getUIFragmentHelper().getTitleBar().setRightMoreTxtColor(ContextCompat.getColor(getActivity(), R.color.color_main_app));
    ...
    private void showDeleteDialog() {
        ...
        mDialog = DialogUtils.getNewMessageDialog(getActivity(), R.drawable.icon_competition_create,"", 0, "删除", "取消",
    ...
    private void setEmptyView(String desc) {
        ...
        ((TextView)mEmptyView.findViewById(R.id.empty_text)).setText(desc);
    }
    ...
    private boolean hasSelectId() {
        List<CompetitionItem> items = adapter.getItems();
        boolean has = false;
    ...
    private void clearAllSelect() {
        ...
        if (items != null && !items.isEmpty()) {
            List<CompetitionItem> unSelectItems = new ArrayList<CompetitionItem>();
            for (CompetitionItem item : items) {
                if (!item.isSelected) {
    ...
    public class CompetitionAdapter extends SingleTypeAdapter<CompetitionItem> {
        GradientDrawable redDeawable = new GradientDrawableBuilder().createCommonDrawable(getActivity())
                .buildColor(getResources().getColor(R.color.color_button_red))
        ...
        GradientDrawable strokeDeawable = new GradientDrawableBuilder().createCommonDrawable(getActivity())
                .buildColor(getResources().getColor(R.color.white))
                .buildStroke(1, getResources().getColor(R.color.color_divider))
                .buildCornerRadius(UIUtils.dip2px(13)).create();
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            ...
            if (convertView == null) {
                ...
                convertView = LayoutInflater.from(mContext).inflate(R.layout.layout_competitions_item, parent, false);
                holder.mItemView = (RelativeLayout) convertView.findViewById(R.id.item_layout);
                holder.mRootView = (RelativeLayout) convertView.findViewById(R.id.root_layout_view);
                layoutParams = (RelativeLayout.LayoutParams) convertView.findViewById(R.id.item_layout).getLayoutParams();
                layoutParams.width = displayWidth;
                convertView.findViewById(R.id.root_layout_view).getLayoutParams().width = displayWidth + UIUtils.dip2px(45);
                holder.mTitle = (TextView) convertView.findViewById(R.id.title);
                holder.mDate = (TextView) convertView.findViewById(R.id.time);
                holder.mGrade = (TextView) convertView.findViewById(R.id.grade);
                holder.mState = (TextView) convertView.findViewById(R.id.state);
                holder.mJoinCount = (TextView) convertView.findViewById(R.id.join_count);
                holder.mStudentCount = (TextView) convertView.findViewById(R.id.student_count);
                holder.mSelectBtn = (ImageView) convertView.findViewById(R.id.select_btn);
            ...
            final CompetitionItem item = getItem(position);
            holder.mTitle.setText(item.name);

            if (CompetitionItem.TYPE_LOOP_MATCH.equals(item.type)) {
                Drawable drawable = ContextCompat.getDrawable(mContext, R.drawable.icon_competition_loop_match);
            ...
            }else if (CompetitionItem.TYPE_SINGLE_TEST.equals(item.type)) {
                Drawable drawable = ContextCompat.getDrawable(mContext, R.drawable.icon_competition_single_test);
                drawable.setBounds(0, UIUtils.dip2px(1), drawable.getMinimumWidth(), drawable.getMinimumHeight() + UIUtils.dip2px(1));
                holder.mTitle.setCompoundDrawables(drawable, null, null, null);
                holder.mTitle.setCompoundDrawablePadding(UIUtils.dip2px(5));
            ...
            if (!item.totalUser.equals("0") && item.totalUser.equals(item.joinUser)) {
                holder.mJoinCount.setTextColor(getResources().getColor(R.color.color_main_app));
                holder.mStudentCount.setTextColor(getResources().getColor(R.color.color_main_app));
            } else {
                holder.mJoinCount.setTextColor(getResources().getColor(R.color.color_text_main));
                holder.mStudentCount.setTextColor(getResources().getColor(R.color.color_text_primary));
            }

            if (item.status == ConstantsUtils.MATCH_STATUS_NOTSTART) {
                holder.mItemView.setBackgroundColor(getResources().getColor(R.color.white));
                holder.mState.setText("未开赛");
                holder.mState.setTextColor(getResources().getColor(R.color.color_text_primary));
                setBackgroundDrawable(holder.mState, strokeDeawable);
            }else if (item.status == ConstantsUtils.MATCH_STATUS_FINISHED) {
                holder.mItemView.setBackgroundColor(getResources().getColor(R.color.color_main_background));
                ...
                holder.mState.setTextColor(getResources().getColor(R.color.white));
