...
public class DirectionalViewPager extends ViewPager {
    ...
    public void setAdapter(PagerAdapter adapter) {
        ...
        if (mAdapter != null) {
            ...
            if (mRestoredCurItem >= 0) {
                ...
                setCurrentItemInternalImpl(mRestoredCurItem, false, true);
                mRestoredCurItem = -1;
    ...
    public void setCurrentItem(int item) {
        mPopulatePending = false;
        setCurrentItemInternalImpl(item, true, false);
    }

    void setCurrentItemInternalImpl(int item, boolean smoothScroll, boolean always) {
        ...
        if (smoothScroll) {
            if (mOrientation == HORIZONTAL) {
                smoothScrollToImpl(getWidth() * item, 0);
            } else {
                smoothScrollToImpl(0, getHeight() * item);
            }
     ...
     */
    void smoothScrollToImpl(int x, int y) {
        if (getChildCount() == 0) {
            // Nothing to do.
            setScrollingCacheEnabled(false);
            return;
    ...
    void addNewItemImpl(int position, int index) {
    ...
    void dataSetChangedImpl() {
        ...
        if (newCurrItem >= 0) {
            setCurrentItemInternalImpl(newCurrItem, false, true);
            needPopulate = true;
        }
        if (needPopulate) {
            populateImpl();
    ...
    public static class SavedState extends BaseSavedState {
        ...
        SavedState(Parcel in, ClassLoader loader) {
            super(in);
            if (loader == null) {
                loader = DirectionalViewPager.class.getClassLoader();
            }
            position = in.readInt();
    ...
    public void onRestoreInstanceState(Parcelable state) {
        ...
        if (mAdapter != null) {
            ...
            setCurrentItemInternalImpl(ss.position, false, true);
        } else {
...
//            }
    ...
    ItemInfo infoForChildImpl(View child) {
        for (int i = 0; i < mItems.size(); i++) {
            ItemInfo ii = mItems.get(i);
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            ItemInfo ii;
            if (child.getVisibility() != GONE && (ii = infoForChildImpl(child)) != null) {
                ...
                if (mIsBeingDragged) {
                    ...
                    if ((Math.abs(initialVelocity) > mMinimumVelocity)
                        ...
                        if (lastMotion > mInitialMotion) {
                            setCurrentItemInternalImpl(mCurItem - 1, true, true);
                        } else {
                            setCurrentItemInternalImpl(mCurItem + 1, true, true);
                        }
                    } else {
                        setCurrentItemInternalImpl(mCurItem, true, true);
                    }
