...
public class GymWordBlankView extends RelativeLayout {
    ...
    protected void onFinishInflate() {
        ...
        mBlankView = (LinearLayout) findViewById(R.id.gym_question_blank_layout);
        mOptionGrid = (LinearLayout) findViewById(R.id.gym_question_blank_gridview);
    }
    ...
    public void updateAnlyze(QuestionItem questionItem) {
        ...
        for (int i = 0; i < blackStr.length(); i++) {
            ...
            if ("_".equals(blackStr.substring(i, i + 1))) {
                ...
                blankTxt.setText("_");
                blankTxt.setTextColor(getResources().getColor(R.color.white));
                blankTxt.setBackgroundResource(R.drawable.icon_gym_blank_wrong);

                answerTxt.setText(String.valueOf(rightAnswer.charAt(position)));
                answerTxt.setTextColor(getResources().getColor(R.color.color_main_app));
